...
#region Java
/**
 * Class GifDecoder - Decodes a GIF file into one or more _frames.
 * <br><pre>
 * Example:
...
namespace GifToSpriteMap.NGif
{
	...
	{
    public event GifDecoderEventHandler FrameAdded;
		...
		protected int[] _gct; // global color table
		protected int[] _lct; // local color table
		...
		protected ArrayList _frames; // _frames read from current file
		...
		public int GetDelay(int n) 
		{
			...
			{
				delay = ((GifFrame) _frames[n]).delay;
			}
			return delay;
		}

		/**
		 * Gets the number of _frames read from file.
		 * @return frame count
		 */
		 ...
		 * _frames as specified by their disposition codes).
		 */
		int [] GetPixels( Bitmap bitmap )
		{
            ...
            for (int th = 0; th < imageHeight; th++)
			{
				for (int tw = 0; tw < imageWidth; tw++)
				{
				...
				if (lastImage != null) 
				{
					...
					if (lastDispose == 2) 
					{
                        ...
                        using (Graphics g = Graphics.FromImage(image))
                        {
                            ...
                            if ( transparency )
                            {
                                c = Color.Empty;// Color.FromArgb(0, 0, 0, 0); // assume background is transparent
...
//                                bool b = c2 == c;
		...
		protected int ReadBlock() 
		{
			...
			if (blockSize > 0) 
			{
				try 
				{
					...
					while (n < blockSize) 
					{
						count = _inStream.Read(block, n, blockSize - n);
						if (count == -1) 
							break;
						n += count;
					}
		...
		protected void ReadHeader() 
		{
			...
			if (gctFlag && !Error()) 
			{
				_gct = ReadColorTable(gctSize);
				bgColor = _gct[bgIndex];
			}
		}
